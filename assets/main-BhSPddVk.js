(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const o of document.querySelectorAll('link[rel="modulepreload"]'))a(o);new MutationObserver(o=>{for(const r of o)if(r.type==="childList")for(const i of r.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&a(i)}).observe(document,{childList:!0,subtree:!0});function s(o){const r={};return o.integrity&&(r.integrity=o.integrity),o.referrerPolicy&&(r.referrerPolicy=o.referrerPolicy),o.crossOrigin==="use-credentials"?r.credentials="include":o.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function a(o){if(o.ep)return;o.ep=!0;const r=s(o);fetch(o.href,r)}})();const b="https://www.themealdb.com/api/json/v1/1",$=document.getElementById("search-form"),w=document.getElementById("search-input"),n=document.getElementById("recipes-container"),p=document.getElementById("loading"),d=document.getElementById("error-message"),L=document.getElementById("home-link"),E=document.getElementById("modalBody");document.getElementById("modalTitle");let g;document.addEventListener("DOMContentLoaded",()=>{g=new bootstrap.Modal(document.getElementById("recipeModal")),u(),L.addEventListener("click",e=>{e.preventDefault(),u()}),$.addEventListener("submit",e=>{e.preventDefault();const t=w.value.trim();t&&I(t)})});function m(){p.style.display="block",n.innerHTML="",d.classList.add("d-none")}function x(){p.style.display="block",d.classList.add("d-none")}function c(){p.style.display="none"}function h(e){d.textContent=e||"An error occurred. Please try again.",d.classList.remove("d-none")}async function l(e){try{const t=`${b}${e}`,s=await fetch(t);if(!s.ok)throw new Error(`Error HTTP: ${s.status}`);return await s.json()}catch(t){throw console.error("Error fetching data:",t),h(t.message),c(),t}}async function u(){m();try{const t=(await l("/categories.php")).categories;n.innerHTML=`
            <div class="row g-4">                        
        ...
        `,document.querySelectorAll(".category-card").forEach(s=>{s.addEventListener("click",()=>{const a=s.getAttribute("data-category");M(a)})})}catch(e){console.error("Error loading home page:",e)}finally{c()}}async function I(e){m();try{const t=await l(`/search.php?s=${encodeURIComponent(e)}`);t.meals?(n.innerHTML=`
              <h2>Results for: "${e}"</h2>
              <div class="row g-4">
          ...
          `}catch(t){console.error("Error searching recipes:",t)}finally{c()}}async function M(e){m();try{const t=await l(`/filter.php?c=${encodeURIComponent(e)}`);t.meals?(n.innerHTML=`
          <br>
          ...
          `}catch(t){console.error("Error fetching recipes by category:",t)}finally{c()}}async function C(e){x();try{const t=await l(`/lookup.php?i=${e}`);if(t.meals&&t.meals.length>0){const s=t.meals[0],a=[];for(let r=1;r<=20;r++){const i=s[`strIngredient${r}`],v=s[`strMeasure${r}`];i&&i.trim()!==""&&a.push(`${v} ${i}`)}const o=s.strInstructions.split(`\r
`).filter(r=>r.trim()!=="").map((r,i)=>`<p>${r}</p>`).join("");E.innerHTML=`
      ...
      `,g.show()}else h("Recipe not found")}catch(t){console.error("Error showing recipe details:",t)}finally{c()}}function f(e){return`
